...
{
    static char buffer[USER_MESSAGE_READ_SIZE];
    static int size = 1;
    static int i = 1;
    char *line = NULL;
    for (int j = 0, new = 1; new != 0; j++) {
        if (size <= i) {
            i = j = 0;
            new = size = read(socket, buffer, USER_MESSAGE_READ_SIZE);
            buffer[size] = '\0';
        }
        if (buffer[i + j] == '\r' && new > 0) {
            line = user_get_message2(line, buffer, i, j);
            i = i + j + 1;
            return (line);
        } else if (buffer[i + j] == '\0' && new > 0) {
            line = user_get_message2(line, buffer, i, j + 1);
            i = size;
        }
    }
    return (line);
